...
const AllCrops = () => {
  ...
  useEffect(() => {
  if (loaderData) {
    setAllCrops(loaderData); 
    setFilteredCrops(loaderData);
  }
}, [loaderData]);
...
const HandleSearch = (e) => {
  ...
  const filtered = allCrops.filter((crop) =>
    crop.cropName.toLowerCase().includes(searchingValue)
  );
  ...
  return (
    <div className="w-full mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        {
           filteredCrops.map(datas=> <AllCropsPage key={datas._id} datas={datas}></AllCropsPage>)
