...
const NavBar = () => {
  ...
  const LogOut = () => {
    logout()
      .then(() => {
        ...
        icon: "success",
        title: "Log-Out",
        text: "Your Log-Out was successfull ✅",
        timer: 2000,
      ...
      Swal.fire({
        icon: "Info",
        title: "Log-Out",
        text: "Your Log-Out was not successfull ",
        timer: 2000,
        showConfirmButton: false,
      });
  ...
  const links = (
    <>
      ...
      <li className="block lg:hidden border-t border-gray-200 mt-2 pt-2">
        {user ? (
          <button
            onClick={LogOut}
            className="w-full text-left px-3 py-1 rounded-md hover:bg-amber-100 transition font-medium text-gray-700"
          >
            Log Out
          </button>
        ) : (
          <NavLink
            to="/login"
            className={({ isActive }) =>
              `block px-3 py-1 rounded-md hover:bg-amber-100 transition ${
                isActive
